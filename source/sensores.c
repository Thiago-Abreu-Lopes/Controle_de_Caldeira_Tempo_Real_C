...
static pthread_mutex_t exclusao_mutua_ALARM = PTHREAD_MUTEX_INITIALIZER; //Mutex dedicado ao alarme
...
/* Chamado pela thread que le o sensor ou atuador e disponibiliza aqui o valor lido */
...
void sensor_alarmeT( double limite) {
	pthread_mutex_lock( &exclusao_mutua_ALARM); 
	limite_atual = limite; 
	while( s_temp < limite_atual) 
		pthread_cond_wait( &alarme, &exclusao_mutua_ALARM); 
	limite_atual = HUGE_VAL; 
	pthread_mutex_unlock( &exclusao_mutua_ALARM); 
}
